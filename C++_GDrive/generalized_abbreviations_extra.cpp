...
void helper(string word, int ind, vector<string> &ans){
	for(int i=1; i<=(int)word.length()-ind-1; i++){
		for(int j=ind+1; j<=(int)word.length()-i; j++){
			...
			if(j+i<word.length())
				temp+=word.substr(j+i); //here j+i and not j+digits[i] coz word is still the same as before and we need to 
			//as many characters ahead as much we replaced.
			ans.push_back(temp);
...
void helper2(int ind, string word, vector<string> &ans){
	if(ind>=word.length())
		return;
	for(int j=ind; j<word.length(); j++){
		for(int i=1; i+j<=word.length(); i++){
			string temp = word.substr(0,j) + to_string(i) + word.substr(j+i);
			ans.push_back(temp);
			helper2(j+(to_string(i)).length()+1, temp, ans);
		}
	}
}

vector<string> func2(string word){
	vector<string> ans;
	helper2(0, word, ans);
	return ans;
}
