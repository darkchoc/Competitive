...
int recurse(vector<int> &zero, vector<int> &one, set<int> &acceptableStates, int &totalLength, int currentState, int currentLength, vector<vector<int> > &dp){
	if(currentLength==totalLength)
		return acceptableStates.find(currentState)!=acceptableStates.end();
	if (dp[currentState][currentLength]!=-1)
		return dp[currentState][currentLength];
	dp[currentState][currentLength] = (recurse(zero, one, acceptableStates, totalLength, zero[currentState], currentLength+1, dp)%mod_ + 
									 recurse(zero, one, acceptableStates, totalLength, one[currentState], currentLength+1, dp)%mod_)%mod_;
	return dp[currentState][currentLength];
}

int recurse2(vector<int> &zero, vector<int> &one, set<int> &acceptableStates, int &totalLength, int currentState, int remainingLength, vector<vector<int> > &dp){
	if(remainingLength==0)
		return acceptableStates.find(currentState)!=acceptableStates.end();
	if(dp[currentState][remainingLength]!=-1)
		return dp[currentState][remainingLength];
	dp[currentState][remainingLength] = (recurse2(zero, one, acceptableStates, totalLength, zero[currentState], remainingLength-1, dp)%mod_ + 
										recurse2(zero, one, acceptableStates, totalLength, one[currentState], remainingLength-1, dp)%mod_)%mod_;
	return dp[currentState][remainingLength];
}

int solve(vector<int> &A, vector<int> &B, vector<int> &C, int D, int E){
	vector<vector<int> > dp(A.size(), vector<int> (E+1, -1));
	set<int> acceptableStates (C.begin(), C.end());
	//return recurse(A, B, acceptableStates, E, D, 0, dp)%mod_;
	return recurse2(A, B, acceptableStates, E, D, E, dp)%mod_;
}
...
int main(){
	...
	cin >> acceptableStates;
	vector<int> acc;
	for(int i=0; i<acceptableStates; i++){
		cin >> x;
		acc.push_back(x);
	}
	int startState;
	...
	int reqdLength;
	cin >> reqdLength;
	cout << solve(Zero, One, acc, startState, reqdLength);
	return 0;
}